...
function ToggleRow({
  ...
  return (
    <div className="flex items-center justify-between border-b border-white/[0.08] py-3 last:border-b-0">
      <div>
        <div className="text-[13px] font-medium">{title}</div>
        <div className="mt-0.5 text-[11px]" style={{ color: "var(--color-fg-3)" }}>{sub}</div>
      </div>
      ...
      <form onSubmit={handleSubmit}>
        <Card title="Voz da marca" sub="Como Agendra fala com seus leads.">
          <Field label="Nome da IA">
            <input className="input" name="ai_name" defaultValue={company?.ai_name ?? "Agendra"} />
          </Field>
          <Field label="Tom">
            <input className="input" name="ai_tone" defaultValue={company?.ai_tone ?? "Próxima, calorosa, com bom português brasileiro"} />
          </Field>
          <Field label="Saudação padrão">
            <input className="input" name="ai_greeting" defaultValue={company?.ai_greeting ?? "Oi! Sou a Agendra 👋"} />
          </Field>
          <Field label="Frases proibidas">
            <input className="input" name="ai_forbidden" defaultValue={company?.ai_forbidden ?? ""} placeholder="Ex.: 'desculpe pelo transtorno'" />
          </Field>
          {error && <p className="mb-2 text-xs text-red-400">{error}</p>}
          <Button type="submit" variant="primary" size="sm" disabled={pending}>
            {saved ? <><Check size={14} /> Salvo ✓</> : pending ? "Salvando…" : "Salvar persona"}
          </Button>
        </Card>
      </form>
...
function Flows() {
  ...
  return (
    <div className="rounded-2xl border border-white/[0.08] bg-white/[0.04] p-5">
      <h3 className="text-base font-semibold">Fluxos ativos</h3>
      <p className="mb-4 mt-1 text-xs" style={{ color: "var(--color-fg-3)" }}>
        Sequências de mensagens disparadas por gatilho.
      </p>
...
function Billing() {
  return (
    <div className="grid gap-4 lg:grid-cols-2">
      <Card title="Plano atual" sub="Pro · Faturado mensalmente">
        <div className="my-3.5 text-4xl font-bold tracking-[-0.03em]">
          R$ 297<span className="text-sm" style={{ color: "var(--color-fg-3)" }}>/mês</span>
        </div>
        <p className="text-xs" style={{ color: "var(--color-fg-3)" }}>Próxima cobrança: 04/06/2026</p>
      ...
      <Card title="Uso este mês" sub="Limites do plano Pro">
        {[
          ...
          >
            <span className="text-xs" style={{ color: "var(--color-fg-2)" }}>{u.lbl}</span>
            ...
            <span className="text-right font-mono text-xs">{u.n}</span>
          </div>
        ))}
      </Card>
    </div>
