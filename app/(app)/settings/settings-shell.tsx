...
// Heaviest, non-default panels — code-split out of the settings initial bundle.
// They lazy-load behind a skeleton, shrinking the JS parsed on first paint.
const PanelSkeleton = () => (
  <div className="flex flex-col gap-4">
    <div className="h-48 animate-pulse rounded-2xl bg-[#F4F4F5]" />
    <div className="h-32 animate-pulse rounded-2xl bg-[#F4F4F5]" />
  </div>
);
const Billing = dynamic(() => import("./settings-billing").then((m) => m.Billing), {
  loading: () => <PanelSkeleton />,
});
const LogsView = dynamic(() => import("./settings-logs").then((m) => m.LogsView), {
  loading: () => <PanelSkeleton />,
});
